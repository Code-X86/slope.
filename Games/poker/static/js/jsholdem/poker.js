...
function my_pseudo_alert (text) {
  var html = "<html><body topmargin=2 bottommargin=0 bgcolor=" +
             ...
             "onclick='window.leave_pseudo_alert()'></form></body></html>";
...
function ask_how_many_opponents () {
  ...
  for (var i = 0; i < 9; i++) {
    if (quick_values[i]) {
      asking += "<font>" +
                "<a href='javascript:window.handle_how_many_reply(" +
...
//  gui_write_modal_box(html9 + html10);
  ...
  javascript:window.handle_how_many_reply(8);
}
...
function main () {
  ...
  if (players[current_bettor_index].status == "BUST" ||
    ...
    if (current_bettor_index == 0) {
      ...
      for (i = 0; i < 6; i++) {
        if (quick_values[i]) {
          quick_bets += "<a href='javascript:window.handle_human_bet(" +
...
function update_func () {
  var url = 'https://sourceforge.net/projects/js-css-poker-fxmod/files/';
  var win = window.open(url, '_blank');
  win.focus();
}
